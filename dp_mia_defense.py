...
model_dp, optimizer_dp, train_loader_A1_dp = privacy_engine.make_private(
    module=model_dp,
...
for epoch in range(epochs_dp):
    model_dp.train()
    total_loss = 0
    for batch in tqdm(train_loader_A1_dp, desc=f"DP Training Epoch {epoch+1}"):
        input_ids = batch['input_ids'].to(device)
        attention_mask = batch['attention_mask'].to(device)
        labels = batch['labels'].to(device)
    ...
    avg_loss = total_loss / len(train_loader_A1_dp)
...
shadow_model_dp, shadow_optimizer_dp, train_loader_B1_dp = privacy_engine_shadow.make_private(
...
print("\n[DP] 새도우 모델 학습 중...")
shadow_model_dp.train()
for batch in tqdm(train_loader_B1_dp, desc="Shadow Model (DP) Training"):
    input_ids = batch['input_ids'].to(device)
    attention_mask = batch['attention_mask'].to(device)
    labels = batch['labels'].to(device)
